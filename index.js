...
const Styles = require('./lib/style.js')(((css, _) => {
	...
	return fs.readFileSync(path.join(process.cwd(), css), 'utf-8')
})(...argv._.filter(v => path.extname(v) == '.css')))

const text = require(path.join(process.cwd(), argv.text))
...
argv._.filter(v => path.extname(v) != '.css').forEach(image => {
	...
	for(let profile in text) {
		...
		const final = path.join(process.cwd(), outputDir, base_out + ext)
