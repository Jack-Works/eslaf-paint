	...
	drawText (text, CSS, settings) {
		...
		const reduceActions = (injections, origin, noAfterHook = false) => {
			const reducePlugins = (fns = []) => {
				let result = [...fns, x => x].reduce((values, fn) => {
					let r = fn(values)
					return r || values
				}, {CSS, text, settings, canvas: this})
			...
			if (!noAfterHook && reducePlugins(injections.after)) return true
		}
		...
		;[
			...
			[Plugins.set.font, x => {
				...
				ctx.font = x.CSS.fontSize + 'px ' + x.CSS.fontFamily}],
